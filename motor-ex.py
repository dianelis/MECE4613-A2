...
SyncForwardR = ?    # the right motor's sync coefficient when it moves forward
SyncBackwardR = ?   # the right motor's sync coefficient when it moves backward
# We now define two standard dictionaries which the script will refer it:
MOTOR = {'R': ck.dc_motor_1, 'L': ck.dc_motor_2}            # dictionary of motors
...
def move(duration=0.3, speed=2, factor_r=1, factor_l=1):
    ...
    Output:
        - the motors' motion
    """
    # set the throttle speed to the right motor first
    ?
